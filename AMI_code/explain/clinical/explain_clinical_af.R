...
int <- function(x){
  qnorm((rank(x,na.last="keep")-0.5)/sum(!is.na(x)))  #int transformation
}
################################## all group

setwd("D:/FangCloudV2/Zheng lab/Zheng lab????????/??????/ACS/ACS_code/explain/linchuang")


meta<-read.delim("linchuang.txt", row.names = 1, sep = '\t', stringsAsFactors = FALSE, check.names = FALSE)
meta_num<-ncol(meta)
...
write.csv(explatin_table,"all_linchuang_explin_table_100_af.csv",row.names = F)
write.csv(t(as.data.frame(colMeans(explatin_table))),"all_linchuang_explin_table_100_mean_af.csv",row.names = F)
...
write.csv(explatin_table,"case_linchuang_explin_table_100_af.csv",row.names = F)
write.csv(t(as.data.frame(colMeans(explatin_table))),"case_linchuang_explin_table_100_mean_af.csv",row.names = F)
...
write.csv(explatin_table,"control_linchuang_explin_table_100_af.csv",row.names = F)
write.csv(t(as.data.frame(colMeans(explatin_table))),"control_linchuang_explin_table_100_mean_af.csv",row.names = F)
