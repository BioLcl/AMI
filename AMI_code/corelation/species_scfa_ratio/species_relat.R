...
heatmap_pdf_group<-function(r_table,outputfile_name="./corr.pdf",bk,width,hight,cluster=F,label_matrix=NA,number_color="black",col_group=NA,ann_colors=NA){
  ...
  pheatmap(r_table, cluster_rows = F,
           ...
           color = c(colorRampPalette(colors =rdbu(100)[1:50])(length(bk)*4/10)[1:round(length(colorRampPalette(colors =rdbu(100)[1:50])(length(bk)*4/10)))],
                     colorRampPalette(colors =rdbu(100)[51:100])(length(bk)*6/10)[1:round(length(colorRampPalette(colors =rdbu(100)[51:100])(length(bk)*6/10))*5/6)]),
           #color=rdbu(100),
           fontsize_row = 7.5,
           fontsize_col = 7.5,
...
######
p_cut<-0.05
setwd("D:/AMI_code/Relat/species_scfa_ratio")
meta<-read.delim("scfa_mol_ratio.txt", row.names = 1, sep = '\t', stringsAsFactors = FALSE, check.names = FALSE)

daixie_num<-ncol(meta)
meta<-as.data.frame(meta)
meta$ID<-rownames(meta)
genus<-read.delim("Species.txt", row.names = 1, sep = '\t', stringsAsFactors = FALSE, check.names = FALSE)
genus<-t(genus)
genus<-as.data.frame(genus)
colnames(genus)
#####
genus_o<-genus
genus_l<-read.delim("species_lefse_sig.txt",  sep = '\t', stringsAsFactors = FALSE, check.names = FALSE,header=F)
...
biaoxing<-biaoxing[biaoxing$ID!="A197",]
...
daixie_imputed<-t(apply(out_daixie,1,impute_daixie))
out_daixie<-daixie_imputed
...
write.csv(lm_all_beta_matrix,"species_scfa_beta.csv")
...
p_cut=0.1
  ...
  corr_sig_log<-get_corr_matrix_sig(lm_all_beta_matrix,fdr_padj,p_cut,r_sig = c(NA,1:6))
  ...
  #Group = c(Case = rgb(7, 53, 104,maxColorValue = 255), control = "#0496ff")
  ann_colors = list(
  ...
  heatmap_pdf_group(corr_sig_r_log,heatmap_sig_cluster,bk,4,4,T,corr_sig_label,"white",col_group,ann_colors)
  dev.off()
