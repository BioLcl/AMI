...
heatmap_pdf_group<-function(r_table,outputfile_name="./corr.pdf",bk,width,hight,cluster=F,label_matrix=NA,number_color="black",col_group=NA,ann_colors=NA){
  ...
  pheatmap(r_table,
          ...
          # angle_col = 315,
           ...
           #color=rdbu(100),
          color = c(colorRampPalette(colors =rdbu(100)[1:50])(length(bk)*4/10)[round(length(colorRampPalette(colors =rdbu(100)[1:50])(length(bk)*5/11))*0.89/4):round(length(colorRampPalette(colors =rdbu(100)[1:50])(length(bk)*4/10)))],
                    colorRampPalette(colors =rdbu(100)[51:100])(length(bk)*6/10)[1:round(length(colorRampPalette(colors =rdbu(100)[51:100])(length(bk)*6/10))*5.36/6)]),
           fontsize_row = 7.5,
           fontsize_col = 7.5,
...
for (i in (biaoxing_num+1):(biaoxing_num+daixie_num)){
  for (j in (biaoxing_num+daixie_num+1):(ncol(lm_all_log))){
    ...
    #if (fit_data_detail[2,5]<0.0001082251){
...
table(lm_raw_p<0.05)
#ttt<-matrix(corr_all_list_log_padj,nrow(lm_all_beta_matrix),ncol(lm_all_beta_matrix),byrow=T)
fdr_padj<-matrix(p.adjust(lm_all_padj_matrix,method="BH"),nrow(lm_all_beta_matrix),ncol(lm_all_beta_matrix))

max(lm_all_beta_matrix)
min(lm_all_beta_matrix)
...
lm_all_padj_matrix<-fdr_padj
  ...
  heatmap_pdf_group(corr_sig_r_log,heatmap_sig_cluster,bk,3.5,4.5,T,corr_sig_label,"white",col_group,ann_colors)
dev.off()
