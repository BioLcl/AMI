...
# read otu table
otuXsample <- otu<-read.delim("Species.txt", row.names = 1, sep = '\t', stringsAsFactors = FALSE, check.names = FALSE,header = T)
otuXsample<-otuXsample/100
dim(otuXsample)
rownames(otuXsample)
min(otuXsample)
...
#综合考虑这些指标选择合适的软阈powers值，使R2可能大但也要保证连通度不能太低。看到网上有教程中提到，一般选择R2大于0.85时的power值
...
# 6.  use dynamic tree to cluster OTU into module .对刚才得到的拓扑矩阵使用相异度dissimilarity between genes对基因进行聚类，然后使用动态剪切法对树进行剪切成不同的模块）
minModuleSize = 5
...
write.csv(KIM,"Species_connectivity.csv")
