...
otuXsample <- otu<-read.delim("Genus_16s.txt", row.names = 1, sep = '\t', stringsAsFactors = FALSE, check.names = FALSE,header = T)
...
dim(otuXsample)
otu_ana<-remove_rare(otuXsample,0.1)
otu_ana[otu_ana==0]=0.000001
otu_ana<-log10(otu_ana)
dim(otu_ana)
...
# 6.  use dynamic tree to cluster OTU into module .对刚才得到的拓扑矩阵使用相异度dissimilarity between genes对基因进行聚类，然后使用动态剪切法对树进行剪切成不同的模块（模块最小基因数为5）
minModuleSize = 5
...
test<-KIM[otu2colors$mergedcolor=="cyan",]


HubGenes <-chooseTopHubInEachModule(sampleXotu_f8,mergedColors,power=softPower)
HubGenes
write.table (HubGenes,file = "HubGenes_of_each_module.xls",quote=F,sep='\t')

save.image("species_final.Rdata")
############
dim(mergedMEs)
dim(sampleXotu_f8)
