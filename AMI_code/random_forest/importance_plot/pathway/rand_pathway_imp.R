...
data<-read.delim("Pathway_importance.txt",header = T)
data<-data[1:10,]
...
gg<-ggplot(data=data, aes(x=ID,y=MeanDecreaseAccuracy))+
  ...
  geom_bar(stat="identity",fill=rgb(3, 172, 218,max=255),width = 0.5)+
...
exp_bar<-gg + theme(panel.grid.major = element_line(colour = "gray71", 
  ...
  labs(x = NULL, y = "Mean Decrease Accuray",title = "Pathway")+
  guides(fill=guide_legend(ncol=2,title = ""))+
  coord_flip()
exp_bar
ggsave("Pathway_imp_col_top10.png",exp_bar,width = 7,height =3.5 )
ggsave("Pathway_imp_col_top10.pdf",exp_bar,width = 7.2,height =3.5 )
