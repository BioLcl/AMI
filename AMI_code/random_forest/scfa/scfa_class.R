...
metadetail<-1
split_number=100
bac<-read.delim("scfa_mol_ratio.txt",header = T,stringsAsFactors = F,check.names = F,row.names = 1)
...
#  trainTransformed<-trainTransformed[,c("class",fs_nb$optVariables)]
  ...
  write.table(imp,paste("./",daixieID,"/","importance.txt",sep=""),sep="\t",quote=F)
...
write.csv(ci.auc(test_roc),"95ci.csv")
